...
<body onload="doConvert()" style="font-size:11px !important;color: black !important;margin:auto;font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif">
  ...
  <table style="width: 100%;" class='striped'>
   ...
   <tbody>
    ...
    while ($custs = mysqli_fetch_array($GetUSERS)) {
     ...
     $GetBookings = SELECT("SELECT * FROM bookings where customer_id='$id' ORDER BY bookingid DESC");
$Bookings = mysqli_fetch_array($GetBookings);
$unit_name = strtoupper($Bookings['unit_name']);
$unit_area = $Bookings['unit_area'];

$PlotNo = $unit_name." (".$unit_area.")";
    ?>
     <tr class='striped'>
      <td><?php echo $customers['id']; ?></td>
      <td><?php echo $customers['name']; ?></td>
      <td><?php echo $PlotNo;?></td>
      <td><?php echo $customers['email']; ?><br><?php echo $customers['phone']; ?></td>
      <td><?php echo $customers['user_city']; ?><br><?php echo $customers['user_state']; ?>-<?php echo $customers['user_pincode']; ?></td>
